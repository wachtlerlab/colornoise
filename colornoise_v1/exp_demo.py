...
class Exp:
    ...
    def __init__(self, subject, par_file, cfg_file, res_dir, priors_file_path):
        ...
        self.Crgb = self.ColorPicker.sml2rgb(self.Csml)
    ...
    def choose_con(self, standard, test):  # choose noise condition
        ...
        if self.param['noise_condition'] == 'L-L':  # low - low noise
            sColor = self.ColorPicker.newcolor(standard)[1]
            tColor = self.ColorPicker.newcolor(test)[1]
    ...
    def run_trial(self, rot, cond, count):
        ...
        disp_standard = self.take_closest(valid_theta, standard)  # theta actually displayed
        if test < 0:
            test += 360
        disp_test = self.take_closest(valid_theta, test)
        sPatch_text = visual.TextStim(self.win, text='actual standard:' + str(disp_standard), units='deg', pos=[0, (-1) ** (rndpos[0][0] < 0) * 10], height=0.5, color='black',
                              colorSpace=self.ColorSpace)
        tPatch_text_1 = visual.TextStim(self.win, text='calculated test:' + str(test), units='deg', pos=[0, (-1) ** (rndpos[1][0] < 0) * 10], height=0.5, color='black',
                              colorSpace=self.ColorSpace)
        tPatch_text_2 = visual.TextStim(self.win, text='actual test:' + str(disp_test), units='deg', pos=[0, (-1) ** (rndpos[1][0] < 0) * 8], height=0.5, color='black',
                              colorSpace=self.ColorSpace)
        ...
        if conditions[0]['stairType'] == 'simple':
            ...
            for cond in conditions:
                ...
                cur_handler = data.QuestHandler(cond['startVal'], cond['startValSd'], pThreshold=cond['pThreshold'],
                                                ...
                                                staircase=prior_handler, extraInfo=cond, grain=0.01)
...
run_exp(subject='psuedo', par_file_path=['config/cn2x3_demo.yaml'], cfg_file_path='config/expconfig_8bit.yaml')
