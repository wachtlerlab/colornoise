...
class ExploreData():
    def __init__(self, sub, sel_par=None, sel_ses=None, rm_ses=None):
        """
    ...
    def fitpf(self):
        ...
        for idx, label in enumerate(allResponses.index):
            ...
            res[label]['fit'] = FitCumNormal(res[label]['combinedInten'],
                                                  res[label]['combinedResp'],
                                                  sems=res[label]['sems'], guess=[0, 4],
                                                  expectedMin=0.5, lapse=0.05)  # customized cumulative Gaussian
    ...
    def threshplot(self, polar=False, savefig=False):
        ...
        if polar is False:
            ...
            thresh += thresh[:1]
            thre_err += thre_err[:1]
            ax.errorbar(shift_angles, thresh, yerr=thre_err, color='grey', ls='--', capsize=5)
    ...
    def rearrange(self, savefig=False):
        ...
        for ii in np.arange(0, len(pool_labels) - 1, 2):
            idx = ii//2
            rearranged.loc[idx, 'hue'] = 'hue_' + str(idx)
            ...
            rearranged.loc[idx, 'fit'] = FitCumNormal(rearranged.loc[idx, 'combinedInten'],
                                         rearranged.loc[idx, 'combinedResp'],
                                         sems=rearranged.loc[idx, 'sems'] , guess=guess,
                                         expectedMin=0.0, lapse=0.01)  # customized cumulative Gaussian
            rearranged.loc[idx, 'pse'] = rearranged.loc[idx, 'fit'].inverse(0.5)
            rearranged.loc[idx, 'thre'] = rearranged.loc[idx, 'fit'].inverse(0.75)
    ...
    def rearrange_pfplot(self):
        ...
        for idx in np.arange(num):
            ...
            ax.plot([-6.0, rearranged.loc[idx, 'pse']],
                    [0.5, 0.5], '--', color='grey')

            ax.plot([rearranged.loc[idx, 'thre'], rearranged.loc[idx, 'thre']],
                    [-6.0, 0.75], '--', color='grey')

            ax.plot([-6.0, rearranged.loc[idx, 'thre']],
                    [0.75, 0.75], '--', color='grey')


            ssq = np.round(hue_fit.ssq, decimals=3)  # sum-squared error
            ax.text(3.5, 0.55, 'ssq = ' + str(ssq), fontsize=fontsize)
            ax.set_title(hue + ' ' + 'threshold = %0.3f' % rearranged.loc[idx, 'thre'], fontsize=fontsize)
            ax.set_ylim([0.0, 1])
            ax.set_xlim([-6, 6])
            ax.tick_params(axis='both', which='major', labelsize=fontsize - 2)
    ...
    def rearrange_paramplot(self):
        ...
        angles = [(n / float(num) * 360) for n in range(num)]
...
# ExploreData('ysu', sel_par=['cn2x8_LL_d', 'cn2x8_LL_e'], sel_ses=['202009']).paramplot()
# ExploreData('ysu', sel_par=['cn2x8_LL_d', 'cn2x8_LL_e']).rearrange_paramplot()