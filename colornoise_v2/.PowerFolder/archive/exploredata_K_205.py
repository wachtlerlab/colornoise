...
class ExploreData():
    ...
    def fitpf(self):
        ...
        for idx, label in enumerate(allResponses.index):
            ...
            res[label]['fit'] = FitCumNormal(res[label]['combinedInten'],
                                                  res[label]['combinedResp'],
                                                  sems=res[label]['sems'], guess=guess,
    ...
    def rearrange(self, savefig=False):
        ...
        for ii in np.arange(len(pool_labels) - 1):
            idx = ii//2
    ...
    def rearrange_pfplot(self):
        ...
        for idx in np.arange(num):
            ...
            for inten, resp, se in zip(rearranged.loc[idx, 'combinedInten'],
                ...
                ax.plot(inten, resp, '.', color=colorcodes[idx], alpha=0.5, markersize=300 / se)
...
# ExploreData('ysu', rm_ses=['0916','0917T10']).pfplot()
# ExploreData('ysu', rm_ses=['0916', '0917', '0918T09']).pfplot()

# ExploreData('ysu',rm_ses=['0918T1', '0921']).pfplot()

# ExploreData('ysu', rm_ses=['0916','0917T10']).rearrange_pfplot()
# ExploreData('ysu', rm_ses=['0916','0917T10']).rearrange_paramplot()

# ExploreData('ysu', sel_par=['cn2x8_HH'], sel_ses_idx=None).rearrange_paramplot()