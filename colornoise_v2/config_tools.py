...
def write_cfg(file_path):
    ...
    cfg_dict['ref.pos'] = [0, 2.5]
    cfg_dict['standard.xlim'] = [-5, -2]
    cfg_dict['standard.ylim'] = [-4, -1]
    cfg_dict['test.xlim'] = [2, 5]
...
def write_par(file_path, noise, method, seed=42, hue_num=8, min_max=None, start_val=5.0, step_type=None,
    ...
    if up_down is None:
        up_down = [None, None]
    """generate stimuli"""
    theta = np.linspace(0+22.5, 360+22.5, hue_num, endpoint=False)
    ...
    for idx, x in enumerate(stim):
        ...
        if (x['label']).endswith('m'):
            par_dict[stim_num]['ref'] = float(x['theta'] - 10)
        else:
            par_dict[stim_num]['ref'] = float(x['theta'] + 10)
...
class WriteXpp:
    def __init__(self, subject, idx, dir_path='data'):
        ...
        if dir_path == 'data':
            ...
            self.file_path = os.path.join(dir_path, subject + idx + '.yaml')
...
def write_xrl(subject, cfg_file=None, par_file=None, xpp_file=None, break_info=None, xls_file=None, dir_path='data'):
    ...
    """
    if dir_path == 'data':
       xrl_path = os.path.join(dir_path, subject, subject + '.xrl')
    else:
       xrl_path = os.path.join(dir_path, subject + '.xrl')

    xrl_file = open(xrl_path, 'a')  # append instead of rewriting
