...
def color4plot(num):
    _, colorcodes = colorpalette.circolors(numStim=num)
...
class ExploreData():
    def __init__(self, sub, parsel=None, onesel=None):
        """
        :param sub: the subject
        :param parsel: the parameter file selector - must be string, e.g. 'cn32rnd_a'
        :param onesel: one single session selector - must be string, e.g. '20191127T1617'
        """
        self.sub = sub
        self.parsel = parsel
        self.onesel = onesel
    ...
    def readxrl(self):
        ...
        """
        xrl = 'config/' + self.sub + '.xrl'
        with open(xrl) as f:
            lines = f.read().splitlines()
            selected = [line for line in lines if line.endswith('.xlsx')]

            if self.parsel is not None:  # select with param arguments
                selected = [line for line in selected if self.parsel in line]
            if self.onesel is not None:  # select with a specific session
                selected = [line for line in selected if self.onesel in line]

            par = [None] * len(selected)
            xls = [None] * len(selected)
            count = 0
            for line in selected:
                par[count] = pd.DataFrame(genconfig.ParReader(line.split(', ')[0]).read_stair())
                xls[count] = pd.read_excel(pd.ExcelFile(line.split(', ')[2]), sheet_name=None)
                count += 1
    ...
    def fitpf(self):
        ...
        plt.savefig('ysu_test_figs/' + self.sub + '_' + self.xstr(self.parsel) + '_' + self.xstr(self.onesel) + '.pdf')
        plt.show()
    ...
    def threshplot(self):
        ...
        plt.savefig('ysu_test_figs/' + self.sub + '_' + 'threshplot_' + str(len(allResponses[1])) + 'trials' + '.pdf')
        plt.show()
...
def fitpf_fromxpp(xpp):
    """
    fit PF directly from xpp files
    :param xpp:
    :return:
    """
    intensities_raw = genconfig.XppReader(xpp).read_customized('diff')
    responses = genconfig.XppReader(xpp).read_customized('response')
    stim = genconfig.XppReader(xpp).read_customized('stim')

    with open(xpp) as xpp_file:
        lines = xpp_file.read().splitlines()
        for line in lines:
            if line.startswith('stimparfl'):
                parfl = line.split(': ')[-1].strip()

    # TODO: the next step would be reading from paramater files
